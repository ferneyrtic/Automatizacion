...
export default function HistorialModal({ user, onClose, points }: HistorialModalProps) {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden border border-[var(--border)]"
      ...
      >
        ...
        <div className="bg-white border-b border-[var(--border)] px-6 py-5">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-[var(--primary)] text-white flex items-center justify-center text-sm font-bold tracking-wide shrink-0">
                {getInitials(user.name)}
              </div>
              <div>
                <h2 className="text-xl font-bold text-[var(--foreground)]">{user.name}</h2>
                <p className="text-sm text-gray-500 mt-0.5">{user.equipo}</p>
              </div>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
          ...
          {participacionCompleta && (
            <div className="mb-4 flex items-center gap-2.5 px-4 py-3 bg-emerald-50 border border-emerald-200 rounded-xl">
                  ...
                  >
                    ...
                    <div className={`px-4 py-2.5 flex items-center justify-between border-b ${
                      ...
                      <div className="flex items-center gap-2.5">
                        <Calendar size={13} className={tieneActividad ? 'text-[var(--primary)]' : 'text-gray-300'} />
                        ...
                        {tieneActividad ? (
                          ...
                          <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-gray-400 bg-gray-100 border border-gray-200 px-2 py-0.5 rounded-full">
                    ...
                    {/* Grid de acciones */}
                    <div className={`p-3 grid grid-cols-3 gap-2 ${expandido ? 'grid' : 'hidden'}`}>
                      {acciones.map((accion, i) => {
                        ...
                        return (
                          ...
                          >
                            <Icon size={15} className={done ? 'text-[var(--primary)]' : 'text-gray-300'} />
                            <div className="flex-1 min-w-0">
                              <p className={`text-xs font-semibold truncate ${done ? 'text-gray-800' : 'text-gray-400'}`}>
                                {accion.label}
                              </p>
          ...
          {/* Resumen final */}
          <div className="mt-4 px-4 py-3 bg-white border border-[var(--border)] rounded-xl flex items-center justify-between text-sm">
        ...
        <div className="p-4 border-t border-[var(--border)] bg-white">
          <button
            onClick={onClose}
            className="w-full py-2.5 bg-[var(--primary)] hover:bg-[var(--primary-dark)] text-white rounded-xl font-semibold text-sm transition-colors"
          >
            Cerrar historial
          </button>
        </div>
...
function StatBox({
  ...
  return (
    <div className="bg-gray-50 border border-[var(--border)] rounded-xl px-4 py-3 text-center">
      <p className={`text-xl font-bold tabular-nums ${color ?? (highlight ? 'text-[var(--primary)]' : 'text-gray-800')}`}>
        {value}
      </p>
